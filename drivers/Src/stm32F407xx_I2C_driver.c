...
/*
 * Helper functions
 *  can be only accessed only is this file
 */
...
void I2C_Init(I2C_Handle_t *pI2CHandle)
{
	...
	pI2CHandle->pI2Cx->TRISE = (tempreg & 0x3F); //masking tempreg as ,Trise require 5 bit value
						//other bits should be in reset sate i.e 0 here to prevent any error
...
void I2C_MasterReceiveData(I2C_Handle_t *pI2CHandle,uint8_t *pRxBuffer, uint8_t Len, uint8_t SlaveAddr,uint8_t Sr)
{
	...
	//procedure to read to read data from slave when LEN >1
	...
	//re-enable ACKing , as we enter this ACK was enable
	// as say user called send data function after this function
	// and that function is based on assumption that ack was enabled earlier
...
/*
 * here we used pI2CHandle->  instead of pI2CHandle.  as  pI2CHandle is of pointer type
 */
...
void I2C_EV_IRQHandling(I2C_Handle_t *pI2CHandle)
{
	...
	//In this we first check which event has caused the interrupt
	//for that we check if the interrupt control bit and flag bit is set for that event for not


	uint32_t temp1, temp2, temp3;

	temp1 = pI2CHandle->pI2Cx->CR2 & (1 << I2C_CR2_ITEVTEN); //to store the value of ITEVTEN in temp1 variable
	temp2 = pI2CHandle->pI2Cx->CR2 & (1 << I2C_CR2_ITBUFEN);//to store the value of ITBUFEN in temp2 variable

	temp3 = pI2CHandle->pI2Cx->SR1 & (1 << I2C_SR1_SB);////to store the value of SB in temp3 variable
	//1. Handle For interrupt generated by SB event
	//	Note : SB flag is only applicable in Master mode
	//		 : in slave mode SB bit is always zero
	//		 : start condition is generated in the function of setting the interrupt
	if(temp1 && temp3)
	{
		//SB flag is set
		//it tells that generation of start condition is success full, we can start the address phase

		//first check the status of I2C if it is in receiving mode then we send the
		// R/W bit as 1, if it is in transmitting mode then we send R/W bit as 0
	...
	if(temp1 && temp3)
	{
		//ADDR flag is set,as it is set the clock will be stretched
		//to restart the clock we have to clear the ADDR flag
		I2C_ClearADDRFlag(pI2CHandle);
	}
	...
	if(temp1 && temp3 )
	{
		//cause of setting of BTF flag also depend on the application state
		if(pI2CHandle->TxRxState == I2C_BUSY_IN_TX)
		{
			//confirm TxE is set
			if(pI2CHandle->pI2Cx->SR1 & (1 << I2C_SR1_TXE))
			{
				...
				//can add another API if LEN != 0 and BTF, TXE becomes one
			}
	...
	//		: This flag is set by hardware when slave detects STOP condition on the bus
	if(temp1 && temp3 )
	{
		//STOF flag is set
	...
	//interrupt happen  due to these event when both the ITEVFEN AND ITBUFEN  are enable
	temp3 = pI2CHandle->pI2Cx->SR1 & (1 << I2C_SR1_TXE);
	//5. Handle For interrupt generated by TXE event
	// NOTE: it indicates that data resister is empty
	//	   : and software has to put more data in the data register
	if(temp1 && temp2 && temp3 )
	{
		//TO CHECK OF DEVICE MODE
...
void I2C_ER_IRQHandling(I2C_Handle_t *pI2CHandle)
{
	...
	temp2 = (pI2CHandle->pI2Cx->CR2) & ( 1 << I2C_CR2_ITERREN);//in this value of ITERREN bit is stored in temp2


/***********************Check for Bus error************************************/

	temp1 = (pI2CHandle->pI2Cx->SR1) & ( 1<< I2C_SR1_BERR);//to store the value of BERR bit in temp1
	if(temp1  && temp2 )//to check if both temp1 and temp2 are positive
	{
		//This is Bus error
...
void I2C_MasterHandleRXNEInterrupt(I2C_Handle_t *pI2CHandle)
{
	...
	{
		//here dereferencing the pointer to store the value at the loaction
 ...
 *
 *****************************************************************/
//closing means we don't want anymore interrupts
